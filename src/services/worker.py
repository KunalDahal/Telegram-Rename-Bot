...
class Worker:
    def __init__(self, task_queue, user_settings_getter, client, config):
        ...
        self.premium_download_limit = max(
            1, min(int(getattr(config, "pm_dl_limit", self.pool_size)), self.pool_size)
        )
        self.premium_upload_limit = max(
            1, min(int(getattr(config, "pm_ul_limit", 1)), self.pool_size)
        )
        ...
        self._premium_upload_slot = asyncio.Semaphore(self.premium_upload_limit)
        ...
        logger.info(
            "[Worker] Concurrency limits: global=%d download=%d premium_download=%d "
            "premium_upload=%d upload=%d watermark=%d",
            self.pool_size,
            self.download_limit,
            self.premium_download_limit,
            self.premium_upload_limit,
            self.upload_limit,
